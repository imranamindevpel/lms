...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $quizDates = $request->input('quiz_date'); // Assuming quiz_date is an array
        ...
        foreach ($foldersArray as $key => $folder) {
            ...
            $userCourse->save();
            Report::where('user_id', $user->id)->where('course_id', $courseIds[$key])->delete();
            $userQuiz = new Report();
            $userQuiz->user_id = $user->id;
            $userQuiz->course_id = $courseIds[$key];
            $userQuiz->quiz_date = $quizDates[$courseIds[$key]]; // Use the corresponding quiz date
            $userQuiz->save();
        }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $courseIds = $request->input('course_ids');
        $quizDates = $request->input('quiz_date');
        if (isset($courseIds) && $courseIds[0] !== null) {
            ...
            foreach ($foldersArray as $key => $folder) {
                ...
                $userQuiz->user_id = $user->id;
                $userQuiz->course_id = $courseIds[$key];
                $userQuiz->quiz_date = $quizDates[$courseIds[$key]]; 
                $userQuiz->save();
            }
        } else {
